...
Deze API moet het mogelijk maken om data op te halen via OData, maar ook data in de vorm van een Digitale Deltaformaat, aan te leveren.
...
Filteren van de data zal gebeuren via OData. 
...
- $top
- $count
- geografisch zoeken: geodistance, ...
...
Toevoegen is toegestaan. Updaten *niet*. Dit om een aantal redenen: 
- Metingen behoren immutable te zijn
- Updaten van data, helemaal indien deze in een bepaalde structuur staan, is zeer inefficient.
- Ook de historie bijhouden wordt ingewikkeld.
Verwijderen is toegestaan.
...
Verwerking zal niet meteen gebeuren. Daarom kan de client een statys bij de server opvragen.
